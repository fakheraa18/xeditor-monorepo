"""Parser for GPT agent mode responses.
...
class HarmonyParser(ResponseParser):
    """
    Parser for GPT agent mode responses.
    ...
    Example:
    <|channel|>commentary to=search_code <|constrain|>json<|message|>{"query":"SIAddEditPage.vue","path":""}
    """
    ...
    # Allows optional numbers (like token counts) between tool name and constrain tag
    # Supports both 'json' and 'code' as constrain types
    # Handles both formats: <|constrain|>json<|message|> and code<|message|>
    HARMONY_TOOL_CALL_START = re.compile(
        r'<\|channel\|>[\w_]+\s+to=(?:functions\.)?([\w_]+)\s+(?:\d+\s+)?(?:<\|constrain\|>)?(?:json|code)<\|message\|>',
        re.IGNORECASE
    )
    ...
    def parse(self, content: str) -> ParsedResponse:
        ...
        # This fixes cases where Harmony token removal strips newlines around code blocks
