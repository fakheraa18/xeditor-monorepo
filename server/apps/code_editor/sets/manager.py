...
from apps.code_editor.prompt_sets import discover_bundled_sets, BUNDLED_SETS_DIR
...
class PromptSet:
    ...
    def read_tools_config(
        ...
        """Read the tools allowlist from apps.code_editor.tools.json. Returns None if not found."""
    ...
    def read_mode_settings(
        ...
        """Read the settings from apps.code_editor.tools.json. Returns empty dict if not found."""
        data = self.read_tools_config_full(family, mode, version)
        if data is None:
            return {}
...
async def handle_get_system_tools(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from apps.code_editor.tools.registry import SYSTEM_TOOLS
