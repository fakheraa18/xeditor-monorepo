  ...
  <div class="inline-editor">
    ...
    <div class="inline-editor-actions row items-center q-gutter-xs q-mt-xs">
      <q-btn
        ...
        @click="handleSubmit"
      />
      <q-btn flat dense size="sm" color="grey" icon="close" label="Cancel" @click="handleCancel" />
    </div>
  </div>
</template>
...
function detectMention(userInput: string): { active: boolean; query: string; startPos: number } {
  ...
  try {
    const inputEl = editorInputRef.value?.$el?.querySelector(
      'textarea',
  ...
  if (showMentionMenu.value && filteredMentions.value.length > 0) {
    ...
    if (event.key === 'ArrowUp') {
      ...
      mentionSelectedIndex.value =
        (mentionSelectedIndex.value - 1 + filteredMentions.value.length) %
