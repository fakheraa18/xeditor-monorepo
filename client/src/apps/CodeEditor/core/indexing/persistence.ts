...
import type { IndexData, IndexMetadata, Symbol, GraphEdge, CodeChunk } from './types';
...
export function saveIndexMetadata(_projectId: ProjectId, _metadata: IndexMetadata): void {
  throw new Error('Index persistence is handled by companion. Direct saving is not supported.');
}

export async function loadIndexMetadata(projectId: ProjectId): Promise<IndexMetadata | null> {
  // Load from companion - requires connection
  const companionStore = useLocalCompanionStore();
  if (!companionStore.isConnected) {
    throw new Error(
      'Local companion connection required to load index metadata. Please ensure the companion server is running.',
    );
  }
...
export function saveVectors(_projectId: ProjectId, _vectors: Map<string, Float32Array>): void {
  throw new Error('Vector persistence is handled by companion. Direct saving is not supported.');
}

export function loadVectors(_projectId: ProjectId): Map<string, Float32Array> {
  throw new Error(
    'Vector loading is handled by companion during retrieval. Direct loading is not supported.',
  );
}
...
export async function loadIndex(projectId: ProjectId): Promise<IndexData | null> {
  ...
  if (!companionStore.isConnected) {
    throw new Error(
      'Local companion connection required to load index. Please ensure the companion server is running.',
    );
  }
...
export async function deleteIndex(projectId: ProjectId): Promise<void> {
  ...
  if (!companionStore.isConnected) {
    throw new Error(
      'Local companion connection required to delete index. Please ensure the companion server is running.',
    );
  }
